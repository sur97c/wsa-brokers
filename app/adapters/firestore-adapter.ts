// File: app/adapters/firestore-adapter.ts
...
export class FirestoreAdapter<T extends FirestoreDocument> {
  ...
  async getDocumentById(documentId: string): Promise<T | null> {
    try {
      const docRef = this.collectionRef.doc(documentId)
      const snapshot = await docRef.get()

      if (!snapshot.exists) {
        throw new DocumentNotFoundError(documentId, this.collectionRef.id)
      }
